# 📊 RegionTrade-Analytics

Автоматическая система аналитики продаж для формирования регулярных отчётов и визуализации ключевых бизнес-метрик.

## 🚀 Возможности

- **Автоматическая обработка данных**: Загрузка, очистка и валидация CSV файлов
- **Расчёт KPI**: Выручка, прибыль, средний чек по бизнес-формулам
- **Генерация отчётов**: 4 типа сводных таблиц (CSV + Excel)
- **Визуализация**: Построение графиков и дашбордов аналитики
- **Логирование**: Детальная трассировка выполнения операций
- **Тестовые данные**: Генератор реалистичных датасетов

## 📊 Формат входных данных

1. Файл продаж data/sales.csv
- date,store_id,order_id,product_id,product_name,category,qty,price,discount,cost
- 2024-01-01,S01,ORD1001,P001,Смартфон iPhone 14,Электроника,1,89999.99,0.1,45000
- 2024-01-01,S01,ORD1001,P002,Наушники AirPods,Электроника,2,19999.99,0,8000

**Обязательные колонки:**

- date (YYYY-MM-DD) - дата продажи
- store_id (S01, S02...) - ID магазина
- order_id - номер заказа
- product_id - ID товара
- product_name - название товара
- category - категория товара
- qty (int) - количество
- price (float) - цена за единицу
- discount (float, 0..1) - доля скидки
- cost (float) - себестоимость за 1 шт.

2. Файл магазинов data/stores.csv
- store_id,city,format
- S01,Москва,offline
- S02,Санкт-Петербург,offline
- S03,Online,online

## 📈 Формируемые отчёты

**Отчёт A: Итоги по дням (output/report_daily.csv)**
  
- date - дата
- total_orders - количество заказов
- total_revenue - общая выручка
- total_profit - общая прибыль
- avg_check - средний чек

**Отчёт B: Итоги по магазинам (output/report_stores.csv)**

- store_id, city, format - информация о магазине
- total_revenue - выручка магазина
- total_profit - прибыль магазина
- orders_count - количество заказов
- top_category - топовая категория по выручке

**Отчёт C: Топ товаров (output/report_top_products.csv)**

**Топ-10 товаров по выручке:**

- product_id, product_name, category
- revenue, profit, qty_sum

**Отчёт D: Категории (output/report_categories.csv)**

- category - категория товара
- revenue - выручка по категории
- profit - прибыль по категории
- share_revenue - доля выручки от общей

## 📊 Визуализации
**Система автоматически строит и сохраняет в папку charts/:**

- Выручка по дням (daily_revenue.png) - линейный график динамики
- Прибыль по дням (daily_profit.png) - линейный график с выделением убытков
- Топ категорий (top_categories.png) - барчарт и круговая диаграмма
- Дашборд аналитики (sales_dashboard.png) - сводная информация на одном изображении

## Cтруктура файлов проекта:
```

project/
│
├── main.py                    # ГЛАВНЫЙ ФАЙЛ - запускает всё
├── generate_test_data.py      # ГЕНЕРАТОР - создаёт тестовые данные
│
├── src/                       # ПАПКА С МОДУЛЯМИ
│   ├── loaders.py            # ЗАГРУЗЧИК - читает CSV файлы
│   ├── cleaning.py           # ОЧИСТИТЕЛЬ - исправляет ошибки в данных
│   ├── metrics.py            # КАЛЬКУЛЯТОР - считает выручку, прибыль
│   ├── reporting.py          # ОТЧЁТЧИК - сохраняет результаты в CSV/Excel
│   └── plotting.py           # ГРАФИКИ - рисует диаграммы и дашборды
│
├── data/                      # ПАПКА С ВХОДНЫМИ ДАННЫМИ
│   ├── sales.csv             # Таблица продаж (что купили)
│   └── stores.csv            # Таблица магазинов (адреса, тип)
│
├── output/                    # ПАПКА С РЕЗУЛЬТАТАМИ
│   ├── report_daily.csv      # Отчёт по дням
│   ├── report_stores.csv     # Отчёт по магазинам
│   ├── report_top_products.csv # Топ товаров
│   ├── report_categories.csv # Отчёт по категориям
│   └── summary_report.xlsx   # Сводный Excel отчёт
│
├── charts/                    # ПАПКА С ГРАФИКАМИ
│   ├── daily_revenue.png     # График выручки по дням
│   ├── daily_profit.png      # График прибыли
│   ├── top_categories.png    # Диаграмма категорий
│   └── sales_dashboard.png   # Дашборд (все графики вместе)
│
└── logs/                      # ПАПКА С ЛОГАМИ
    └── analysis.log          # Лог файл (что программа делала)
```
## 🔧 Очистка данных
**Система автоматически выполняет:**

- ✅ Приведение дат к типу datetime
- ✅ Исправление отрицательных значений (qty, price, cost)
- ✅ Корректировку скидок в диапазон 0..1
- ✅ Заполнение пропусков категории значением "Unknown"
- ✅ Удаление дубликатов и некорректных строк

Статистика очистки записывается в лог-файл logs/analysis.log.

## 🧮 Расчёт метрик
**Формулы KPI:**

**Выручка**
- revenue = qty * price * (1 - discount)

**Прибыль**  
- profit = revenue - qty * cost

Пример расчёта:
- Товар: 2 шт × 1000 руб × (1 - 0.1 скидка) = 1800 руб выручки

- Прибыль: 1800 - (2 × 500 себестоимость) = 800 руб

## 📊 Пример вывода в консоль

📈 ОБЩАЯ СТАТИСТИКА:
   - Период: 01.01.2024 - 31.01.2024
   - Всего выручка: 648,957.81 руб.
   - Всего прибыль: 214,902.66 руб.
   - Средний чек: 695.56 руб.
   - Всего заказов: 933

🏪 ТОП-3 МАГАЗИНА ПО ВЫРУЧКЕ:
   1. S02 (Екатеринбург, offline) - 120,042.61 руб.
   2. S03 (Online, online) - 118,636.32 руб.
   3. S05 (Воронеж, offline) - 115,434.23 руб.

🏷️ ТОП-3 КАТЕГОРИИ:
   1. Электроника - 30.32% (196,772.30 руб.)
   2. Спорттовары - 14.82% (96,190.97 руб.)
   3. Продукты - 14.52% (94,217.10 руб.)


## 🔧 Архитектура модулей
**loaders.py - Загрузка данных**
- Загрузка CSV файлов с проверкой формата
- Валидация обязательных колонок
- Обработка ошибок файловой системы

**cleaning.py - Очистка данных**
- Исправление некорректных значений
- Обработка пропусков
- Статистика очистки

**metrics.py - Расчёт метрик**
- Расчёт выручки и прибыли для каждой строки
- Агрегация по дням, магазинам, категориям
- Поиск топовых товаров и категорий

**reporting.py - Формирование отчётов**
- Создание CSV отчётов
- Генерация Excel файла со сводкой
- Консольный вывод итогов

<img width="1920" height="1043" alt="Снимок экрана (877)" src="https://github.com/user-attachments/assets/c190184c-8f29-4266-b868-8f16a2be3f97" />


**plotting.py - Визуализация**
- Построение графиков выручки и прибыли
<img width="2284" height="1077" alt="top_categories" src="https://github.com/user-attachments/assets/68060352-4205-4275-ae98-5366fb24b56d" />
<img width="2684" height="1768" alt="sales_dashboard" src="https://github.com/user-attachments/assets/1c668385-732a-472f-bf06-89e811a11acd" />




## 📋 Технические требования

- Python 3.10+
- Pandas 1.5.0+
- Matplotlib 3.6.0+
- Openpyxl 3.0.0+ (для Excel отчётов)
- Создание дашборда аналитики
- Сохранение изображений в PNG
